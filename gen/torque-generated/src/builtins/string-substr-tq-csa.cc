...
TF_BUILTIN(StringPrototypeSubstr, CodeStubAssembler) {
  ...
  if (block11.is_used()) {
    ...
    tmp16 = CodeStubAssembler(state_).WordEqual(TNode<UintPtrT>{phi_bb11_12}, TNode<UintPtrT>{tmp15});
  ...
  if (block14.is_used()) {
    ca_.Bind(&block14);
    tmp18 = CodeStubAssembler(state_).UintPtrAdd(TNode<UintPtrT>{phi_bb3_9}, TNode<UintPtrT>{phi_bb11_12});
    tmp19 = StringBuiltinsAssembler(state_).SubString(TNode<String>{tmp1}, TNode<UintPtrT>{phi_bb3_9}, TNode<UintPtrT>{tmp18});
