...
TF_BUILTIN(RegExpStringIteratorPrototypeNext, CodeStubAssembler) {
  ...
  if (block39.is_used()) {
    ...
    tmp52 = ca_.UncheckedCast<BoolT>(CodeStubAssembler(state_).DecodeWord32FromWord<base::BitField<bool, 34, 1, uintptr_t>>(ca_.UncheckedCast<WordT>(ca_.BitcastTaggedToWordForTagAndSmiBits(tmp4))));
  ...
  if (block10.is_used()) {
    ...
    tmp68 = FromConstexpr_intptr_constexpr_int31_0(state_, 40);
    ...
    tmp71 = ca_.UncheckedCast<Smi>(ca_.BitcastWordToTaggedSigned(CodeStubAssembler(state_).UpdateWord32InWord<base::BitField<bool, 32, 1, uintptr_t>>(ca_.UncheckedCast<WordT>(ca_.BitcastTaggedToWordForTagAndSmiBits(tmp69)), ca_.UncheckedCast<Uint32T>(tmp70))));
