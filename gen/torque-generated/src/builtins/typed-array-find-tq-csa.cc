...
tmp9 = TORQUE_CAST(CodeStubAssembler(state_).CallBuiltinPointer(Builtins::CallableFor(ca_.isolate(),ExampleBuiltinForTorqueFunctionPointerType(1)).descriptor(), tmp2, TNode<Object>(), tmp8, phi_bb14_8));
  ...
  return TNode<Object>{phi_bb19_5};
}
...
TF_BUILTIN(TypedArrayPrototypeFind, CodeStubAssembler) {
  ...
  if (block11.is_used()) {
    ...
    tmp13 = FindAllElements_0(state_, TNode<Context>{parameter0}, TorqueStructAttachedJSTypedArrayAndLength_0{TNode<JSTypedArray>{tmp2}, TNode<UintPtrT>{tmp3}}, TNode<JSReceiver>{tmp7}, TNode<Object>{tmp12});
